...
//    double PCFreq = double(timestart.QuadPart)/1000.0;
    float approx_fps = 0;
    int cntr = 0;
...
//            printf("%f ms/frame with z_cap = %d\n", 1000.0/double(nbFrames), z_cap);
//            //printf("%d FPS with z_cap = %d\n", nbFrames, z_cap);
//            nbFrames = 0;
//            lastTime += 1.0;
//        }
...
//        double tmpassed = double(li.QuadPart - timestart.QuadPart)/PCFreq;
//        double curfps = 1.0 / tmpassed;
//        timestart = li;
//        PCFreq = double(timestart.QuadPart)/1000.0;
//        printf("%f time passed, %f FPS\n", tmpassed, curfps);
        ...
        par->draw(textureEnable, twoside, animationEnable, lightEnable);
...
void keyCallback(GLFWwindow *window, int key, int scancode, int action, int mods){
    ...
    if (action == GLFW_PRESS || action == GLFW_REPEAT){
        switch (key){
            ...
            case GLFW_KEY_F:
                if (scene.is_open()){
                    scene << "This is a line\n";
                    scene << "This is another line\n";
                    scene.close();
                } else {
